...
parallel B-pack change (#348), which affects only the parallel path; the serial
path is untouched and measured within noise of the original (57.83 → 57.30
GFLOP/s at one thread). The single-threaded sweeps were therefore deliberately
not re-run, so those sections are unchanged.
...
`gemm_rect` improved from 6.43× to **7.07×** and `ewise` from 5.13× to **7.12×**
with the parallel B pack (#348) — both route through the same blocked GEMM.
