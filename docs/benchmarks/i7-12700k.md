...
- **Multi-core GEMM scaling improved** — native-fast now reaches **6.51× on 8
  P-cores** (previously 5.82×), narrowing the gap to OpenBLAS at 8 threads from
  ~62% to ~70%.
...
- **Native sparse LU trails SuperLU by 2–18×**, and the gap grows with fill —
  exactly the supernodal kernel-class gap the epic predicts.
...
reaches 6.51× (81% parallel efficiency), and the absolute gap to OpenBLAS at 8
threads narrows from ~62% to **70.3%**.
...
Ordered by ratio, this is the clearest result on the page: **the gap grows
monotonically with fill**. Native wins on the small, sparse `add32` (3.3×
faster, and with *half* the fill). It loses by 2× on the `wang` pair, 7.5× on
`raefsky3`, and 18× on `bbmat`.

The fill ratio explains it. Where native produces less fill than SuperLU (0.50–0.53
on `add32`/`wang3`/`wang4`) the time gap stays near 2×. Where native produces
*more* fill — 1.16× on `raefsky3`, **2.49× on `bbmat`** — the gap explodes. So
native is losing twice over on dense-fill problems: it does more arithmetic, and
it does that arithmetic with scalar non-supernodal kernels instead of BLAS-3.
Closing this is precisely the supernodal work the epic tracks.
